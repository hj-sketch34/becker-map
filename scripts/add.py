...
What this does, in plain English:
  ...
  single accomplishment. This script:
    ...
    2. skips it if that source link is already on the map (no duplicates),
...
ALLOWED_CATEGORIES = {
    ...
    "community",  # shows of support — the senator showing up for constituents
}
...
def existing_sources():
    """Source URLs already in the database, so we never add the same pin twice."""
    if not ACCOMPLISHMENTS_FILE.exists():
        return set()
    with ACCOMPLISHMENTS_FILE.open() as f:
        return {row["source_url"].strip() for row in csv.DictReader(f)}
...
def coords_for_source(source):
    """Read back the lat/lon the database now holds for this source link (or '', '')."""
    if not ACCOMPLISHMENTS_FILE.exists():
        return "", ""
    with ACCOMPLISHMENTS_FILE.open() as f:
        for row in csv.DictReader(f):
            if row["source_url"].strip() == source:
                return row["latitude"].strip(), row["longitude"].strip()
    return "", ""


def remove_source_row(source):
    """Take a row back out of the database — used when a pin fails a guardrail."""
    with ACCOMPLISHMENTS_FILE.open() as f:
        reader = csv.DictReader(f)
        rows = [r for r in reader if r["source_url"].strip() != source]
...
def main():
    ...
    if source in existing_sources():
        sys.exit(f"Already on the map (same source link) — nothing added:\n  {source}")
    ...
    row = {
        "id": source,  # the source URL doubles as the unique id (matches existing rows)
